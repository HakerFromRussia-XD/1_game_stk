...
#ifndef HEADER_CGUIFLUXARA_DRIFTListBox_HPP
#define HEADER_CGUIFLUXARA_DRIFTListBox_HPP
...
namespace irr
{
    ...
    class CGUIFLUXARA_DRIFTListBox : public IGUIElement
    {
    public:
            ...
            //! constructor
            CGUIFLUXARA_DRIFTListBox(IGUIEnvironment* environment, IGUIElement* parent,
                    s32 id, core::rect<s32> rectangle, bool clip=true,
                    bool drawBack=false, bool moveOverSelect=false);

            //! destructor
            virtual ~CGUIFLUXARA_DRIFTListBox();
            ...
            // Touch scrolling in legacy FLUXARA_DRIFT lists is deliberately conservative:
            // it waits for a third of a row and stops at finger-up. Screens
            // that present a mobile feed can opt into native-style dragging
            // and a short decelerating continuation instead.
            void setTouchInertiaEnabled(bool enabled);
            float getTouchScrollPosition() const;
    ...
    private:
            ...
            bool m_deactivated;
            bool m_touch_inertia_enabled;
            float m_touch_scroll_velocity;
            float m_touch_scroll_position;
            uint64_t m_last_touch_time;
            uint64_t m_last_inertia_time;

            void updateTouchInertia();
            float getTouchScrollMaximum() const;
            void setTouchScrollPosition(float position, bool rubber_band);
    };
