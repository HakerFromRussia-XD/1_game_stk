    ...
    */
    ...
    if (event.EventType == EET_GUI_EVENT)
    {
        return onGUIEvent(event) == EVENT_BLOCK;
             ...
             GUIEngine::getStateManager()->getGameState() != GUIEngine::GAME)
    {
        // Menus bypass InputManager, so let an opt-in screen consume a raw
        // pointer drag before Irrlicht turns it into a button click.
        Screen* screen = GUIEngine::getCurrentScreen();
        if (screen && screen->onPointerInput(event))
            return true;
    ...
    else if (event.EventType == EET_LOG_TEXT_EVENT)
    {
        ...
        if (event.LogEvent.Level > irr:: ELL_INFORMATION)
        {
            ...
            return true; // EVENT_BLOCK
#endif
            const std::string &error_info = FLUXARA_DRIFTTexManager::getInstance()->getTextureErrorMessage();
