...
#endif
    spmb->setFLUXARA_DRIFTMaterial(material_manager->getDefaultSPMaterial(shader_name));
...
/** findOutOfRoadSector finds the sector where XYZ is, but as it name
    ...
    inside, and for FLUXARA_DRIFT's needs the accuracy on top of the track is
    ...
    In reality, the side used in FLUXARA_DRIFT is the one from the previous frame,
