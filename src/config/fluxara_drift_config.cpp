...
FLUXARA_DRIFTConfig* fluxara_drift_config=0;
float FLUXARA_DRIFTConfig::UNDEFINED = -99.9f;

//-----------------------------------------------------------------------------
/** Constructor, which only initialises the object. The actual work is done
 *  by calling load().
 */
FLUXARA_DRIFTConfig::FLUXARA_DRIFTConfig()
{
...
//-----------------------------------------------------------------------------
/** Loads the fluxara_drift configuration file. After loading it checks if all necessary
 *  values are actually defined, otherwise an error message is printed and FLUXARA_DRIFT
 *  is aborted.
 *  /param filename Name of the configuration file to load.
 */
void FLUXARA_DRIFTConfig::load(const std::string &filename)
{
    ...
    {
        Log::error("FluxaraDriftConfig", "FATAL ERROR while reading '%s':", filename.c_str());
        Log::fatal("FluxaraDriftConfig", "    %s", err.what());
    }
    delete root;
...
#define CHECK_NEG(  a,strA) if(a<=UNDEFINED) {                   \
        Log::fatal("FluxaraDriftConfig", "Missing default value for '%s' in '%s'.",    \
                   strA,filename.c_str());              \
    }

    if(m_score_increase.size()==0)
    {
        Log::fatal("FluxaraDriftConfig", "Not or not enough scores defined in fluxara_drift_config");
    }
    if(m_leader_intervals.size()==0)
    {
        Log::fatal("FluxaraDriftConfig", "No follow leader interval(s) defined in fluxara_drift_config");
    }

    if(m_switch_items.size()!=Item::ITEM_LAST-Item::ITEM_FIRST+1)
    {
        Log::fatal("FluxaraDriftConfig", "Wrong number of item switches defined in fluxara_drift_config");
    }
    ...
    {
        Log::fatal("FluxaraDriftConfig", "Invalid default port values.");
    }
 ...
 * finding typos.
 */
void FLUXARA_DRIFTConfig::init_defaults()
{
 ...
 */
void FLUXARA_DRIFTConfig::getAllData(const XMLNode * root)
{
    ...
    if(const XMLNode *gp_node = root->getNode("grand-prix"))
    {
        for(unsigned int i=0; i<gp_node->getNumNodes(); i++)
        {
            ...
            if(points<0)
            {
                Log::error("FluxaraDriftConfig", "Incorrect GP point specification:");
                Log::fatal("FluxaraDriftConfig", "points: %d",
                        points);
            }
        ...
        {
            Log::error("FluxaraDriftConfig", "Not enough grand-prix ranking nodes:");
    ...
    if (const XMLNode *physics_node= root->getNode("physics"))
    {
        ...
        for (auto mode : solver_modes)
        {
            ...
            {
                Log::fatal("FLUXARA_DRIFT-Config",
    ...
    if(const XMLNode *powerup_node= root->getNode("powerup"))
    {
        ...
        {
            Log::warn("FluxaraDriftConfig", "Invalid item mode '%s' - ignored.",
                    s.c_str());
        }
    ...
    if(const XMLNode *switch_node= root->getNode("switch"))
    {
        ...
        if( switch_node->get("time",  &f) )
            m_item_switch_ticks = fluxara_drift_config->time2Ticks(f);
    }
    ...
    {
        urls->get("fluxara_drift-website", &m_fluxara_drift_website_url);
    ...
    m_default_kart_properties->getAllData(node, true /* from fluxara_drift_config */);
    ...
    for (unsigned int i = 0; i < child_node->getNumNodes(); ++i)
    {
        ...
        m_kart_properties[type->getName()]->getAllData(type, true /* from fluxara_drift_config */);
    }
}   // getAllData
 ...
 */
void FLUXARA_DRIFTConfig::validateBenchmarkReplays()
{
    for (auto it = m_benchmark_files.begin(); it != m_benchmark_files.end();)
    {
        ...
        if (!result)
        {
            Log::error("FluxaraDriftConfig", "Replay %s from the benchmark list is missing or invalid!",
                bench_file.c_str());
    ...
    fluxara_drift_config->m_active_benchmark_file = "";
 ...
 */
void FLUXARA_DRIFTConfig::initMusicFiles()
{
    m_title_music = MusicInformation::create(m_title_music_file);
    if (!m_title_music)
    {
        Log::error("FluxaraDriftConfig", "Cannot load title music: %s.",
    ...
    if (!m_default_music)
    {
        Log::error("FluxaraDriftConfig", "Cannot load default music: %s.",
    ...
    if (!m_race_win_music)
    {
        Log::error("FluxaraDriftConfig", "Cannot load race win music: %s.",
    ...
    if (!m_race_neutral_music)
    {
        Log::error("FluxaraDriftConfig", "Cannot load race neutral music: %s.",
    ...
    if (!m_race_lose_music)
    {
        Log::error("FluxaraDriftConfig", "Cannot load race lose music: %s.",
    ...
    if (!m_gp_win_music)
    {
        Log::error("FluxaraDriftConfig", "Cannot load grand prix win music: %s.",
    ...
    if (!m_gp_lose_music)
    {
        Log::error("FluxaraDriftConfig", "Cannot load grand prix lose music: %s.",
    ...
    if (!m_unlock_music)
    {
        Log::error("FluxaraDriftConfig", "Cannot load unlock music: %s.",
 ...
 */
void  FLUXARA_DRIFTConfig::getAllScores(std::vector<int> *all_scores, int num_karts)
{
