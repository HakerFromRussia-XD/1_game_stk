...
#ifndef HEADER_FLUXARA_DRIFT_CONFIG_HPP
#define HEADER_FLUXARA_DRIFT_CONFIG_HPP
...
/**
 * \brief Global FLUXARA_DRIFT configuration information.
 ...
 * \ingroup config
 */
class FLUXARA_DRIFTConfig : public NoCopy
{
protected:
...
public:
    ...
    /* URLs for the fluxara_drift website, donating and reseting the password */
    std::string m_fluxara_drift_website_url;
    ...
    /** Lists of TTF files used in FLUXARA_DRIFT. */
...
private:
    /** True if fluxara_drift_config has been loaded. This is necessary if the
     *  --fluxara_drift-config command line parameter has been specified to avoid
     *  that fluxara_drift loads the default configuration after already having
...
extern FLUXARA_DRIFTConfig* fluxara_drift_config;
#endif
