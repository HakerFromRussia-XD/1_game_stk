...
/*! \file fluxara_drift_peer.hpp
 *  \brief Defines functions to easily manipulate 8-bit network destinated strings.
 */

#ifndef FLUXARA_DRIFT_PEER_HPP
#define FLUXARA_DRIFT_PEER_HPP
 ...
 */
class FLUXARA_DRIFTPeer : public NoCopy
{
protected:
...
public:
    FLUXARA_DRIFTPeer(ENetPeer *enet_peer, FLUXARA_DRIFTHost* host, uint32_t host_id);
    // ------------------------------------------------------------------------
    ~FLUXARA_DRIFTPeer();
    // ------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------
    bool isSamePeer(const FLUXARA_DRIFTPeer* peer) const;
    ...
    float getConnectedTime() const
       { return float(FluxaraDriftTime::getMonoTimeMs() - m_connected_time) / 1000.0f; }
    // ------------------------------------------------------------------------
    ...
    {
        int64_t diff =
            (int64_t)FluxaraDriftTime::getMonoTimeMs() - m_last_activity.load();
        if (diff < 0)
            return 0;
    ...
    void updateLastMessage()
                   { m_last_message.store((int64_t)FluxaraDriftTime::getMonoTimeMs()); }
    // ------------------------------------------------------------------------
