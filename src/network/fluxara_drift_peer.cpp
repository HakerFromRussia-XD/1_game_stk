 ...
 */
FLUXARA_DRIFTPeer::FLUXARA_DRIFTPeer(ENetPeer *enet_peer, FLUXARA_DRIFTHost* host, uint32_t host_id)
       : m_address(enet_peer->address), m_host(host)
{
    ...
    m_host_id             = host_id;
    m_connected_time      = FluxaraDriftTime::getMonoTimeMs();
    ...
    m_last_activity.store((int64_t)FluxaraDriftTime::getMonoTimeMs());
    m_last_message.store(0);
    m_consecutive_messages = 0;
}   // FLUXARA_DRIFTPeer

//-----------------------------------------------------------------------------
FLUXARA_DRIFTPeer::~FLUXARA_DRIFTPeer()
{
}   // ~FLUXARA_DRIFTPeer

//-----------------------------------------------------------------------------
void FLUXARA_DRIFTPeer::disconnect()
{
 ...
 */
void FLUXARA_DRIFTPeer::sendPacket(NetworkString *data, bool reliable, bool encrypted)
{
    if (m_disconnected.load())
        return;
    ...
    if (packet)
    {
        ...
        {
            Log::verbose("FLUXARA_DRIFTPeer", "sending packet of size %d to %s at %lf",
 ...
 */
bool FLUXARA_DRIFTPeer::isConnected() const
{
    Log::debug("FLUXARA_DRIFTPeer", "The peer state is %i", m_enet_peer->state);
    return (m_enet_peer->state == ENET_PEER_STATE_CONNECTED);
}   // isConnected

//-----------------------------------------------------------------------------
/** Returns if this FLUXARA_DRIFTPeer is the same as the given peer.
 */
bool FLUXARA_DRIFTPeer::isSamePeer(const FLUXARA_DRIFTPeer* peer) const
{
...
/** Returns if this FLUXARA_DRIFTPeer is the same as the given peer.
*/
bool FLUXARA_DRIFTPeer::isSamePeer(const ENetPeer* peer) const
{
 ...
 */
uint32_t FLUXARA_DRIFTPeer::getPing()
{
    ...
    if (NetworkConfig::get()->isServer())
    {
        ...
        // Frequency is 10 packets per second as seen in FLUXARA_DRIFTHost
...
//-----------------------------------------------------------------------------
void FLUXARA_DRIFTPeer::setCrypto(std::unique_ptr<Crypto>&& c)
{
    m_crypto = std::move(c);
}   // setCrypto
