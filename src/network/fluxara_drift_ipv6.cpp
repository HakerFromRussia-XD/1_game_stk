...
static int
fluxara_drift_inet_pton4(const char *src, unsigned char *dst)
{
...
static int
fluxara_drift_inet_pton6(const char *src, void *dest)
{
    ...
    while((ch = *src++) != '\0')
    {
        ...
        if(ch == '.' && ((tp + INADDRSZ) <= endp) &&
            fluxara_drift_inet_pton4(curtok, tp) > 0)
        {
            tp += INADDRSZ;
            saw_xdigit = 0;
            break;    /* '\0' was seen by fluxara_drift_inet_pton4(). */
        }
        return (0);
    }
    if(saw_xdigit)
...
// ============================================================================
// Android FLUXARA_DRIFT seems to crash when using inet_ntop so we copy it from linux
static const char *
fluxara_drift_inet_ntop4(const u_char *src, char *dst, socklen_t size)
{
...
static const char *
fluxara_drift_inet_ntop6(const uint8_t *src, char *dst, socklen_t size)
{
    /*
    ...
    */
    ...
    for (i = 0; i < 8; i++)
    {
        ...
        {
            if (!fluxara_drift_inet_ntop4(src + 12, tp, sizeof tmp - (tp - tmp)))
                return (NULL);
            tp += strlen(tp);
            break;
        }
...
std::string getIPV6ReadableFromIn6(const struct sockaddr_in6* in)
{
    ...
    fluxara_drift_inet_ntop6(in->sin6_addr.s6_addr, &ipv6[0], INET6_ADDRSTRLEN);
...
extern "C" int64_t upperIPv6(const char* ipv6)
{
    ...
    if (fluxara_drift_inet_pton6(ipv6, &v6_in) != 1)
        return 0;
    uint64_t result = 0;
    unsigned shift = 56;
...
extern "C" int insideIPv6CIDR(const char* ipv6_cidr, const char* ipv6_in)
{
    ...
    if (mask_location == NULL)
        return 0;
    struct in6_addr v6_in;
    if (fluxara_drift_inet_pton6(ipv6_in, &v6_in) != 1)
        return 0;

    char ipv6[INET6_ADDRSTRLEN] = {};
    memcpy(ipv6, ipv6_cidr, mask_location - ipv6_cidr);
    struct in6_addr cidr;
    if (fluxara_drift_inet_pton6(ipv6, &cidr) != 1)
        return 0;
