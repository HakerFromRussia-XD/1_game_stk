...
FLUXARA_DRIFTHost *FLUXARA_DRIFTHost::m_fluxara_drift_host[PT_COUNT];
bool     FLUXARA_DRIFTHost::m_enable_console = false;

std::shared_ptr<LobbyProtocol> FLUXARA_DRIFTHost::create(ChildLoop* cl)
{
    ...
    if (NetworkConfig::get()->isServer())
    {
        ...
        m_fluxara_drift_host[pt] = new FLUXARA_DRIFTHost(true/*server*/);
        sl->initServerStatsTable();
        lp = sl;
    }
    else
    {
        m_fluxara_drift_host[pt] = new FLUXARA_DRIFTHost(false/*server*/);
    }
    // Separate process for client-server gui if exists
    m_fluxara_drift_host[pt]->m_client_loop = cl;
    if (cl)
    {
        m_fluxara_drift_host[pt]->m_client_loop_thread = std::thread(
            std::bind(&ChildLoop::run, cl));
    }
    if (!m_fluxara_drift_host[pt]->m_network)
    {
 ...
 *  closely together with the fluxara_drift server: a (game) server first connects
 *  to the fluxara_drift server and registers itself, clients find the list of servers
 *  from the fluxara_drift server. They insert a connections request into the fluxara_drift
 ...
 *  the fluxara_drift server are used:
 ...
 *
 *    1. ServerLobby:
 *       1. Register this server with fluxara_drift server (i.e. publish its public
 ...
 *       2. The server lobby now polls the fluxara_drift server for client connection
 ...
 *  Client:
 *
 *    The GUI queries the fluxara_drift server to get a list of available servers
 ...
 *    randomly), and then instantiates FLUXARA_DRIFTHost with the id of this server.
 *    FLUXARA_DRIFTHost then triggers ConnectToServer, which do the following:
 *       1. Register the client with the FLUXARA_DRIFT host ('join-server-key' command,
 ...
 *  "0xffff" and "aloha-fluxara_drift", so the client can use the intercept in enet for
 ...
 */
FLUXARA_DRIFTHost::FLUXARA_DRIFTHost(bool server)
{
    ...
    if (server)
    {
        ...
        {
            Log::warn("FLUXARA_DRIFTHost", "Disable IPv6 socket due to missing IPv6.");
            setIPv6Socket(0);
        }
#endif
        addr.port = ServerConfig::m_server_port;
        if (addr.port == 0 && !UserConfigParams::m_random_server_port)
            addr.port = fluxara_drift_config->m_server_port;
    ...
    if (!m_network)
    {
        Log::fatal("FLUXARA_DRIFTHost", "An error occurred while trying to create an "
                              "ENet server host.");
    }
    if (server)
        Log::info("FLUXARA_DRIFTHost", "Server port is %d", getPrivatePort());
}   // FLUXARA_DRIFTHost
 ...
 */
void FLUXARA_DRIFTHost::init()
{
    m_players_in_game.store(0);
    m_players_waiting.store(0);
    m_total_players.store(0);
    m_network_timer.store((int64_t)FluxaraDriftTime::getMonoTimeMs());
    m_shutdown         = false;
    m_authorised       = false;
    m_network          = NULL;
    ...
    {
        Log::error("FLUXARA_DRIFTHost", "Could not initialize enet.");
        return;
    }

    Log::info("FLUXARA_DRIFTHost", "Host initialized.");
 ...
 */
FLUXARA_DRIFTHost::~FLUXARA_DRIFTHost()
{
 ...
 *  down.
 */
void FLUXARA_DRIFTHost::shutdown()
{
 ...
 */
BareNetworkString FLUXARA_DRIFTHost::getStunRequest(uint8_t* stun_tansaction_id)
{
...
//-----------------------------------------------------------------------------
void FLUXARA_DRIFTHost::getIPFromStun(int socket, const std::string& stun_address,
                            short family, SocketAddress* result)
{
    ...
    if (len <= 0)
    {
        Log::error("FLUXARA_DRIFTHost", "STUN response contains no data at all");
        return;
    }
    ...
    if (response.size() < 20)
    {
        Log::error("FLUXARA_DRIFTHost", "STUN response should be at least 20 bytes.");
        return;
    }

    if (response.getUInt16() != 0x0101)
    {
        Log::error("FLUXARA_DRIFTHost", "STUN has no binding success response.");
        return;
    }
    ...
    {
        Log::error("FLUXARA_DRIFTHost", "STUN response doesn't contain the magic "
            "cookie");
        return;
    }
    ...
    for (int i = 0; i < 12; i++)
    {
        ...
        {
            Log::error("FLUXARA_DRIFTHost", "STUN response doesn't contain the "
                "transaction ID");
            return;
        }
    }
    ...
    while (true)
    {
        ...
        if (type == mapped_address || type == xor_mapped_address)
        {
            if (response.size() < 2)
            {
                Log::error("FLUXARA_DRIFTHost", "Invalid STUN mapped address length.");
                return;
            }
    ...
    if (!xor_addr.isUnset() || !non_xor_addr.isUnset())
    {
        ...
        {
            Log::warn("FLUXARA_DRIFTHost", "Only non xor-mapped address returned.");
            *result = non_xor_addr;
        }
    }
}   // getIPFromStun
 ...
 */
void FLUXARA_DRIFTHost::setPublicAddress(short family)
{
    ...
    {
        Log::warn("FLUXARA_DRIFTHost", "Failed to get enough stun servers using SRV"
            " record.");
    }
    ...
    while (!untried_server.empty() && !ProtocolManager::lock()->isExiting())
    {
        ...
        Log::debug("FLUXARA_DRIFTHost", "Using STUN server %s", server_name.c_str());
        uint64_t ping = FluxaraDriftTime::getMonoTimeMs();
        ...
        if (!result.isUnset())
        {
            ...
            ping = FluxaraDriftTime::getMonoTimeMs() - ping;
            // Succeed, save ping
        ...
        else
        {
            ...
            // from FLUXARA_DRIFT then it will be re-added next time, and FLUXARA_DRIFT team will
...
*/
void FLUXARA_DRIFTHost::disconnectAllPeers(bool timeout_waiting)
{
    ...
    if (!m_peers.empty() && timeout_waiting)
    {
        ...
        m_exit_timeout.store(FluxaraDriftTime::getMonoTimeMs() + 2000);
    }
    m_peers.clear();
}   // disconnectAllPeers
 ...
 */
void FLUXARA_DRIFTHost::setErrorMessage(const irr::core::stringw &message)
{
    if (!message.empty())
    {
        Log::error("FLUXARA_DRIFTHost", "%s", StringUtils::wideToUtf8(message).c_str());
    }
 ...
 */
void FLUXARA_DRIFTHost::startListening()
{
    m_exit_timeout.store(std::numeric_limits<uint64_t>::max());
    m_listening_thread = std::thread(std::bind(&FLUXARA_DRIFTHost::mainLoop, this,
        FLUXARA_DRIFTProcess::getType()));
}   // startListening
 ...
 */
void FLUXARA_DRIFTHost::stopListening()
{
 ...
 */
void FLUXARA_DRIFTHost::mainLoop(ProcessType pt)
{
    std::string thread_name = "FLUXARA_DRIFTHost";
    if (pt == PT_CHILD)
        thread_name += "_child";
    VS::setThreadName(thread_name.c_str());

    FLUXARA_DRIFTProcess::init(pt);
    Log::info("FLUXARA_DRIFTHost", "Listening has been started.");
    ENetEvent event;
    ...
    {
        ENetAddress eaddr = {};
        eaddr.port = fluxara_drift_config->m_server_discovery_port;
        direct_socket = new Network(1, 1, 0, 0, &eaddr);
        if (direct_socket->getENetHost() == NULL)
        {
            Log::warn("FLUXARA_DRIFTHost", "No direct socket available, this "
    ...
    uint64_t last_ping_time = FluxaraDriftTime::getMonoTimeMs();
    uint64_t last_update_speed_time = FluxaraDriftTime::getMonoTimeMs();
    uint64_t last_ping_time_update_for_client = FluxaraDriftTime::getMonoTimeMs();
    std::map<std::string, uint64_t> ctp;
    while (m_exit_timeout.load() > FluxaraDriftTime::getMonoTimeMs())
    {
        // Clear outdated connect to peer list every 15 seconds
        for (auto it = ctp.begin(); it != ctp.end();)
        {
            if (it->second + 15000 < FluxaraDriftTime::getMonoTimeMs())
                it = ctp.erase(it);
            else
                it++;
        }

        if (last_update_speed_time < FluxaraDriftTime::getMonoTimeMs())
        {
            // Update upload / download speed per second
            last_update_speed_time = FluxaraDriftTime::getMonoTimeMs() + 1000;
        ...
        if (direct_socket && sl && sl->waitingForPlayers())
        {
            ...
            {
                Log::warn("FLUXARA_DRIFTHost", "Direct socket error: %s",
                    e.what());
            }
        }   // if discovery host
        ...
        if (is_server)
        {
            ...
            if (sl && (!sl->isRacing() || sl->allowJoinedPlayersWaiting()) &&
                last_ping_time < FluxaraDriftTime::getMonoTimeMs())
            {
            ...
            if (need_ping)
            {
                ...
                for (auto& p : m_peers)
                {
                        ...
                        p.second->getConnectedTime() > 5.0f && ap > max_ping)
                    {
                        ...
                        {
                            Log::info("FLUXARA_DRIFTHost", "%s %s with ping %d is higher"
            ...
            for (auto it = m_peers.begin(); it != m_peers.end();)
            {
                ...
                {
                    Log::info("FLUXARA_DRIFTHost", "%s has not been validated for more"
        ...
        for (auto& p : copied_list)
        {
            ...
            case ECT_RESET:
                ...
                // Remove the fluxara_drift peer of it
                std::lock_guard<std::mutex> lock(m_peers_mutex);
                m_peers.erase(peer);
                break;
            }
        ...
        while (enet_host_service(host, &event, 10) != 0)
        {
            ...
            if (!is_server &&
                last_ping_time_update_for_client < FluxaraDriftTime::getMonoTimeMs())
            {
                last_ping_time_update_for_client =
                    FluxaraDriftTime::getMonoTimeMs() + 2000;
                if (lp && lp->isRacing())
                {
            ...
            Event* fluxara_drift_event = NULL;
            if (event.type == ENET_EVENT_TYPE_CONNECT)
            {
                // ++m_next_unique_host_id for unique host id for database
                auto fluxara_drift_peer = std::make_shared<FLUXARA_DRIFTPeer>
                    (event.peer, this, ++m_next_unique_host_id);
                std::unique_lock<std::mutex> lock(m_peers_mutex);
                m_peers[event.peer] = fluxara_drift_peer;
                size_t new_peer_count = m_peers.size();
                lock.unlock();
                fluxara_drift_event = new Event(&event, fluxara_drift_peer);
                Log::info("FLUXARA_DRIFTHost", "%s has just connected. There are "
                    "now %u peers.", fluxara_drift_peer->getAddress().toString().c_str(),
                    new_peer_count);
                // Client always trust the server
                if (!is_server)
                    fluxara_drift_peer->setValidated(true);
            ...
            else if (event.type == ENET_EVENT_TYPE_DISCONNECT)
            {
                ...
                // Use the previous fluxara_drift peer so protocol can see the network
                ...
                {
                    std::shared_ptr<FLUXARA_DRIFTPeer>& peer = m_peers.at(event.peer);
                    addr = peer->getAddress().toString();
                    fluxara_drift_event = new Event(&event, peer);
                    m_peers.erase(event.peer);
                    new_peer_count = m_peers.size();
                }
                Log::info("FLUXARA_DRIFTHost", "%s has just disconnected. There are "
                    "now %u peers.", addr.c_str(), new_peer_count);
            }   // ENET_EVENT_TYPE_DISCONNECT

            std::unique_lock<std::mutex> lock(m_peers_mutex);
            if (!fluxara_drift_event && m_peers.find(event.peer) != m_peers.end())
            {
                std::shared_ptr<FLUXARA_DRIFTPeer> peer = m_peers.at(event.peer);
                lock.unlock();
                if (isPingPacket(event.packet->data, event.packet->dataLength))
                {
                    if (!is_server)
                    {
                        ...
                        {
                            // For old server
                            Log::debug("FLUXARA_DRIFTHost", "%s", e.what());
                        }
                        if (client_ping > 0)
                        {
                            assert(m_nts);
                ...
                {
                    Log::warn("FLUXARA_DRIFTHost", "%s", e.what());
                    enet_packet_destroy(event.packet);
                    continue;
                }
            }
            else if (!fluxara_drift_event)
            {
                enet_packet_destroy(event.packet);
                continue;
            }
            if (fluxara_drift_event->getType() == EVENT_TYPE_MESSAGE)
            {
                Network::logPacket(fluxara_drift_event->data(), true);
#ifdef DEBUG_MESSAGE_CONTENT
                Log::verbose("NetworkManager",
                             "Message, Sender : %s time %f message:",
                             fluxara_drift_event->getPeer()->getAddress()
                             .toString(/*show port*/false).c_str(),
                             FluxaraDriftTime::getRealTime());
                Log::verbose("NetworkManager", "%s",
                             fluxara_drift_event->data().getLogMessage().c_str());
#endif
            }   // if message event
            ...
            if (pm && !pm->isExiting())
                pm->propagateEvent(fluxara_drift_event);
            else
                delete fluxara_drift_event;
        }   // while enet_host_service
    }   // while m_exit_timeout.load() > FluxaraDriftTime::getMonoTimeMs()
    delete direct_socket;
    Log::info("FLUXARA_DRIFTHost", "Listening has been stopped.");
}   // mainLoop
 ...
 *  FLUXARA_DRIFT server). It checks for any messages (i.e. a LAN broadcast requesting
 ...
 */
void FLUXARA_DRIFTHost::handleDirectSocketRequest(Network* direct_socket,
                                        std::shared_ptr<ServerLobby> sl,
                                        std::map<std::string, uint64_t>& ctp)
{
    const int LEN=2048;
    ...
    if (command == "fluxara_drift-server")
    {
        Log::verbose("FLUXARA_DRIFTHost", "Received LAN server query");
    ...
    else if (command == connection_cmd)
    {
        ...
        if (!sender.isLAN() && !sender.isPublicAddressLocalhost() &&
            !NetworkConfig::get()->isPublicServer())
        {
            Log::error("FLUXARA_DRIFTHost", "Client trying to connect from '%s'",
                peer_addr.c_str());
            Log::error("FLUXARA_DRIFTHost", "which is outside of LAN - rejected.");
            return;
        }
        if (ctp.find(peer_addr) == ctp.end())
        {
            ctp[peer_addr] = FluxaraDriftTime::getMonoTimeMs();
            std::make_shared<ConnectToPeer>(sender)->requestStart();
        }
    }
    else if (command == "fluxara_drift-server-port")
    {
        BareNetworkString s;
        s.addUInt16(getPrivatePort());
        direct_socket->sendRawPacket(s, sender);
        ...
        Log::info("FLUXARA_DRIFTHost", "Received unknown command '%s'",
 ...
 */
bool FLUXARA_DRIFTHost::peerExists(const SocketAddress& peer)
{
    std::lock_guard<std::mutex> lock(m_peers_mutex);
    for (auto p : m_peers)
    {
        auto fluxara_drift_peer = p.second;
        if (fluxara_drift_peer->getAddress() == peer ||
            ((fluxara_drift_peer->getAddress().isPublicAddressLocalhost() &&
            peer.isPublicAddressLocalhost()) &&
            fluxara_drift_peer->getAddress().getPort() == peer.getPort()))
            return true;
    }
    return false;
}   // peerExists
 ...
 *  \return FLUXARA_DRIFTPeer the FLUXARA_DRIFTPeer of server.
 */
std::shared_ptr<FLUXARA_DRIFTPeer> FLUXARA_DRIFTHost::getServerPeerForClient() const
{
 ...
 */
void FLUXARA_DRIFTHost::sendPacketToAllPeersInServer(NetworkString *data, bool reliable)
{
    std::lock_guard<std::mutex> lock(m_peers_mutex);
    for (auto p : m_peers)
    {
 ...
 *  \param reliable If the data should be sent reliable or now.
 */
void FLUXARA_DRIFTHost::sendPacketToAllPeers(NetworkString *data, bool reliable)
{
    std::lock_guard<std::mutex> lock(m_peers_mutex);
    for (auto p : m_peers)
    {
 ...
 *  \param reliable If the data should be sent reliable or now.
 */
void FLUXARA_DRIFTHost::sendPacketExcept(FLUXARA_DRIFTPeer* peer, NetworkString *data,
                               bool reliable)
{
    std::lock_guard<std::mutex> lock(m_peers_mutex);
    for (auto p : m_peers)
    {
        FLUXARA_DRIFTPeer* fluxara_drift_peer = p.second.get();
        if (!fluxara_drift_peer->isSamePeer(peer) && p.second->isValidated() &&
            !p.second->isWaitingForGame())
        {
            fluxara_drift_peer->sendPacket(data, reliable);
        }
    }
}   // sendPacketExcept
 ...
 *  \param reliable If the data should be sent reliable or now.
 */
void FLUXARA_DRIFTHost::sendPacketToAllPeersWith(std::function<bool(FLUXARA_DRIFTPeer*)> predicate,
                                       NetworkString* data, bool reliable)
{
    ...
    {
        FLUXARA_DRIFTPeer* fluxara_drift_peer = p.second.get();
        if (!fluxara_drift_peer->isValidated())
            continue;
        if (predicate(fluxara_drift_peer))
            fluxara_drift_peer->sendPacket(data, reliable);
    }
...
void FLUXARA_DRIFTHost::sendToServer(NetworkString *data, bool reliable)
{
    std::lock_guard<std::mutex> lock(m_peers_mutex);
    if (m_peers.empty())
        return;
...
std::vector<std::shared_ptr<NetworkPlayerProfile> >
    FLUXARA_DRIFTHost::getAllPlayerProfiles() const
{
...
//-----------------------------------------------------------------------------
std::set<uint32_t> FLUXARA_DRIFTHost::getAllPlayerOnlineIds() const
{
...
//-----------------------------------------------------------------------------
std::shared_ptr<FLUXARA_DRIFTPeer> FLUXARA_DRIFTHost::findPeerByHostId(uint32_t id) const
{
    std::lock_guard<std::mutex> lock(m_peers_mutex);
    auto ret = std::find_if(m_peers.begin(), m_peers.end(),
        [id](const std::pair<ENetPeer*, std::shared_ptr<FLUXARA_DRIFTPeer> >& p)
        {
...
//-----------------------------------------------------------------------------
std::shared_ptr<FLUXARA_DRIFTPeer>
    FLUXARA_DRIFTHost::findPeerByName(const core::stringw& name) const
{
    std::lock_guard<std::mutex> lock(m_peers_mutex);
    auto ret = std::find_if(m_peers.begin(), m_peers.end(),
        [name](const std::pair<ENetPeer*, std::shared_ptr<FLUXARA_DRIFTPeer> >& p)
        {
            bool found = false;
...
//-----------------------------------------------------------------------------
void FLUXARA_DRIFTHost::initClientNetwork(ENetEvent& event, Network* new_network)
{
    assert(NetworkConfig::get()->isClient());
    ...
    auto fluxara_drift_peer = std::make_shared<FLUXARA_DRIFTPeer>(event.peer, this,
        m_next_unique_host_id++);
    fluxara_drift_peer->setValidated(true);
    m_peers[event.peer] = fluxara_drift_peer;
    auto pm = ProtocolManager::lock();
    if (pm && !pm->isExiting())
        pm->propagateEvent(new Event(&event, fluxara_drift_peer));
}   // initClientNetwork

// ----------------------------------------------------------------------------
std::pair<int, int> FLUXARA_DRIFTHost::getAllPlayersTeamInfo() const
{
    int red_count = 0;
    int blue_count = 0;
...
std::vector<std::shared_ptr<NetworkPlayerProfile> >
    FLUXARA_DRIFTHost::getPlayersForNewGame(bool* has_always_on_spectators) const
{
    ...
    for (auto& p : m_peers)
    {
        ...
        if (has_always_on_spectators && fluxara_drift_peer->alwaysSpectate())
        {
            *has_always_on_spectators = true;
            fluxara_drift_peer->setWaitingForGame(false);
            fluxara_drift_peer->setSpectator(true);
            continue;
        }
        if (fluxara_drift_peer->isWaitingForGame())
            continue;
        if (ServerConfig::m_ai_handling && fluxara_drift_peer->isAIPeer())
            continue;
        for (auto& q : fluxara_drift_peer->getPlayerProfiles())
            players.push_back(q);
    }
    return players;
 ...
 */
void FLUXARA_DRIFTHost::updatePlayers(unsigned* ingame, unsigned* waiting,
                            unsigned* total)
{
    ...
    for (auto& p : m_peers)
    {
        ...
        if (ServerConfig::m_ai_handling && fluxara_drift_peer->isAIPeer())
            continue;
        if (fluxara_drift_peer->isWaitingForGame())
            waiting_players += (uint32_t)fluxara_drift_peer->getPlayerProfiles().size();
        else
            ingame_players += (uint32_t)fluxara_drift_peer->getPlayerProfiles().size();
        total_players += (uint32_t)fluxara_drift_peer->getPlayerProfiles().size();
    }
...
/** True if this is a client and server in graphics mode made by server
  *  creation screen. */
bool FLUXARA_DRIFTHost::isClientServer() const
{
...
std::string FLUXARA_DRIFTHost::getValidPublicAddress() const
{
...
// ----------------------------------------------------------------------------
int FLUXARA_DRIFTHost::receiveRawPacket(char *buffer, int buffer_len,
...
// ----------------------------------------------------------------------------
void FLUXARA_DRIFTHost::sendRawPacket(const BareNetworkString &buffer,
...
// ----------------------------------------------------------------------------
uint16_t FLUXARA_DRIFTHost::getPrivatePort() const
{
