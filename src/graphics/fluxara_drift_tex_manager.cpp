...
// ----------------------------------------------------------------------------
FLUXARA_DRIFTTexManager::~FLUXARA_DRIFTTexManager()
{
#ifndef SERVER_ONLY
...
#endif
}   // ~FLUXARA_DRIFTTexManager

// ----------------------------------------------------------------------------
video::ITexture* FLUXARA_DRIFTTexManager::findTextureInFileSystem(const std::string& filename,
                                                   std::string* full_path)
{
    ...
    if (relative_path.empty())
    {
        if (!m_texture_error_message.empty())
            Log::error("FLUXARA_DRIFTTexManager", "%s", m_texture_error_message.c_str());
        Log::error("FLUXARA_DRIFTTexManager", "Failed to load %s.", filename.c_str());
        return NULL;
    }
    *full_path =
...
// ----------------------------------------------------------------------------
video::ITexture* FLUXARA_DRIFTTexManager::getTexture(const std::string& path,
                                std::function<void(video::IImage*)> image_mani)
{
    if (path.empty())
    {
        Log::error("FLUXARA_DRIFTTexManager", "Texture name is empty.");
        return NULL;
    }
    ...
    if (new_texture->loadingFailed())
    {
        ...
        if (!m_texture_error_message.empty())
        {
            Log::error("FLUXARA_DRIFTTexManager", "%s",
                m_texture_error_message.c_str());
        }
        Log::error("FLUXARA_DRIFTTexManager", "Texture %s not found or invalid.",
            name);
...
// ----------------------------------------------------------------------------
video::ITexture* FLUXARA_DRIFTTexManager::addTexture(video::ITexture* texture)
{
...
// ----------------------------------------------------------------------------
bool FLUXARA_DRIFTTexManager::removeTexture(video::ITexture* texture, bool remove_all)
{
    bool ret = false;
#ifdef DEBUG
    ...
    {
        Log::error("FLUXARA_DRIFTTexManager", "%s undeleted!", s.c_str());
    }
#endif
    if (remove_all)
        return true;
 ...
 */
void FLUXARA_DRIFTTexManager::setTextureErrorMessage(const std::string &error,
...
// ----------------------------------------------------------------------------
int FLUXARA_DRIFTTexManager::dumpTextureUsage()
{
    int size = 0;
    for (auto p : m_all_textures)
    {
    ...
    Log::info("FLUXARA_DRIFTTexManager", "Total %dMB", size);
    return size;
}   // dumpTextureUsage

// ----------------------------------------------------------------------------
bool FLUXARA_DRIFTTexManager::hasTexture(const std::string& path)
{
    if (path.empty())
    {
        Log::error("FLUXARA_DRIFTTexManager", "Texture name is empty.");
...
// ----------------------------------------------------------------------------
void FLUXARA_DRIFTTexManager::reloadAllTextures(bool mesh_texture_only)
{
#ifndef SERVER_ONLY
    GE::GEVulkanDriver* gevd = GE::getVKDriver();
    if (gevd)
    ...
    for (auto p : m_all_textures)
    {
        ...
        if (mesh_texture_only)
        {
            ...
            for (auto& mtp : mesh_texture_paths)
            {
                ...
                {
                    Log::info("FLUXARA_DRIFTTexManager","%s reloaded", full_path.c_str());
                    p.second->reload();
                    break;
                }
            }
