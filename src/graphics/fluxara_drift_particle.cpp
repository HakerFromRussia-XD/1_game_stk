...
// ----------------------------------------------------------------------------
std::vector<float> FLUXARA_DRIFTParticle::m_flips_data;
GLuint FLUXARA_DRIFTParticle::m_flips_buffer = 0;
// ----------------------------------------------------------------------------
FLUXARA_DRIFTParticle::FLUXARA_DRIFTParticle(bool randomize_initial_y, ISceneNode* parent, s32 id,
...
// ----------------------------------------------------------------------------
void FLUXARA_DRIFTParticle::generateParticlesFromPointEmitter
...
// ----------------------------------------------------------------------------
void FLUXARA_DRIFTParticle::generateParticlesFromBoxEmitter
...
// ----------------------------------------------------------------------------
void FLUXARA_DRIFTParticle::generateParticlesFromSphereEmitter
...
// ----------------------------------------------------------------------------
static bool isFLUXARA_DRIFTParticleType(scene::E_PARTICLE_EMITTER_TYPE type)
{
    switch (type)
    {
...
}   // isFLUXARA_DRIFTParticleType

// ----------------------------------------------------------------------------
void FLUXARA_DRIFTParticle::setEmitter(scene::IParticleEmitter* emitter)
{
    CParticleSystemSceneNode::setEmitter(emitter);
    if (!emitter || !isFLUXARA_DRIFTParticleType(emitter->getType()))
    {
...
// ----------------------------------------------------------------------------
void FLUXARA_DRIFTParticle::generate(std::vector<CPUParticle>* out)
{
    if (!getEmitter())
    {
        return;
...
// ----------------------------------------------------------------------------
void FLUXARA_DRIFTParticle::stimulateHeightMap(float dt, unsigned int active_count,
...
// ----------------------------------------------------------------------------
void FLUXARA_DRIFTParticle::stimulateNormal(float dt, unsigned int active_count,
                                  std::vector<CPUParticle>* out)
{
    const core::matrix4 cur_matrix = AbsoluteTransformation;
...
// ----------------------------------------------------------------------------
void FLUXARA_DRIFTParticle::updateFlips(unsigned maximum_particle_count)
{
    bool updated = false;
...
// ----------------------------------------------------------------------------
void FLUXARA_DRIFTParticle::OnRegisterSceneNode()
{
    if (CVS->isGLSL())
    {
        Log::warn("FLUXARA_DRIFTParticle", "Don't call OnRegisterSceneNode with GLSL");
        return;
    }
    generate(NULL);
    Particles.clear();
