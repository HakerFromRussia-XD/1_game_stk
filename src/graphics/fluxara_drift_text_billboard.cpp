...
// ----------------------------------------------------------------------------
FLUXARA_DRIFTTextBillboard::FLUXARA_DRIFTTextBillboard(const video::SColor& color_top,
                             ...
                             core::vector3df(0.0f, 0.0f, 0.0f), scale)
{
    ...
    m_ge_node = NULL;
}   // FLUXARA_DRIFTTextBillboard

// ----------------------------------------------------------------------------
float FLUXARA_DRIFTTextBillboard::getDefaultScale(FontWithFace* face)
{
    return 1.0f / (float)face->getDPI() / face->getNativeScalingFactor();
}   // getDefaultScale

// ----------------------------------------------------------------------------
void FLUXARA_DRIFTTextBillboard::updateAbsolutePosition()
{
...
// ----------------------------------------------------------------------------
void FLUXARA_DRIFTTextBillboard::init(const core::stringw& text, FontWithFace* face)
{
    m_face = face;
    m_text = text;
    m_chars = new std::vector<FLUXARA_DRIFTTextBillboardChar>();
...
// ----------------------------------------------------------------------------
void FLUXARA_DRIFTTextBillboard::initLegacy(const core::stringw& text, FontWithFace* face)
{
    removeGENode();
    m_face = face;
    m_text = text;
...
// ----------------------------------------------------------------------------
void FLUXARA_DRIFTTextBillboard::render()
{
...
// ----------------------------------------------------------------------------
void FLUXARA_DRIFTTextBillboard::OnRegisterSceneNode()
{
...
// ----------------------------------------------------------------------------
void FLUXARA_DRIFTTextBillboard::collectChar(video::ITexture* texture,
                                   const core::rect<float>& dest_rect,
                                   const core::rect<s32>& source_rect,
                                   const video::SColor* const colors)
{
    assert(m_chars != NULL);
    m_chars->push_back(FLUXARA_DRIFTTextBillboardChar(texture, dest_rect, source_rect,
        colors));
}   // collectChar

// ----------------------------------------------------------------------------
void FLUXARA_DRIFTTextBillboard::reload()
{
    clearBuffer();
    if (CVS->isGLSL())
        init(m_text, m_face);
...
static void updateTextBillboard(core::array<scene::ISceneNode*>& List)
{
    ...
    {
        if (FLUXARA_DRIFTTextBillboard* tb = dynamic_cast<FLUXARA_DRIFTTextBillboard*>(List[i]))
            tb->reload();
...
// ----------------------------------------------------------------------------
void FLUXARA_DRIFTTextBillboard::updateAllTextBillboards()
{
    updateTextBillboard(
...
// ----------------------------------------------------------------------------
void FLUXARA_DRIFTTextBillboard::removeGENode()
{
    if (m_ge_node)
    {
...
// ----------------------------------------------------------------------------
void FLUXARA_DRIFTTextBillboard::clearBuffer()
{
