...
export NDK_TOOLCHAIN_PATH="$NDK_PATH/toolchains/llvm/prebuilt/$NDK_HOST_TAG/bin"
export NDK_SYSROOT="$NDK_PATH/toolchains/llvm/prebuilt/$NDK_HOST_TAG/sysroot"
...
build_deps()
{
    ...
    if [ ! -f "$DIRNAME/deps-$ARCH_OPTION/mbedtls.stamp" ]; then
        ...
        if [ "$ARCH_OPTION" = "x86" ]; then
            sed_in_place 's/#define MBEDTLS_AESNI_C//g' "$DIRNAME/deps-$ARCH_OPTION/mbedtls/include/mbedtls/mbedtls_config.h"
        fi
    ...
    if [ ! -f "$DIRNAME/deps-$ARCH_OPTION/astc-encoder.stamp" ]; then
        ...
        sed_in_place '/-Werror/d' Source/cmake_core.cmake
        ...
        if [ "$ARCH_OPTION" = "armeabi-v7a" ]; then
            ...
            if [ "$ARCH_OPTION" = "x86" ]; then
                sed_in_place 's/_mm_popcnt_u64/__builtin_popcountll/g' Source/astcenc_vecmathlib_sse_4.h
            fi
            cmake . -DCMAKE_TOOLCHAIN_FILE=../../../cmake/Toolchain-android.cmake \
                    -DHOST=$HOST -DARCH=$ARCH -DCMAKE_C_FLAGS="-fpic -O3 -g"      \
                    -DCMAKE_CXX_FLAGS="-fpic -O3 -g"                              \
    ...
    if [ "$ARCH_OPTION" = "arm64-v8a" ]; then
        if [ ! -f "$DIRNAME/deps-$ARCH_OPTION/libadrenotools.stamp" ]; then
            ...
            if [ -d "$DIRNAME/../lib/libadrenotools/.git" ]; then
                ...
                cp -a "$DIRNAME/../lib/libadrenotools" "$DIRNAME/deps-$ARCH_OPTION/libadrenotools"
            fi
...
[binaries]
ar = ndk_path / 'toolchains/llvm/prebuilt/$NDK_HOST_TAG/bin/llvm-ar'
c = ['ccache', ndk_path / 'toolchains/llvm/prebuilt/$NDK_HOST_TAG/bin/aarch64-linux-android26-clang']
cpp = ['ccache', ndk_path / 'toolchains/llvm/prebuilt/$NDK_HOST_TAG/bin/aarch64-linux-android26-clang++', '-fno-exceptions', '-fno-unwind-tables', '-fno-asynchronous-unwind-tables', '-static-libstdc++', '-Wno-c++11-narrowing']
c_ld = 'lld'
cpp_ld = 'lld'
...
strip = [ndk_path / 'toolchains/llvm/prebuilt/$NDK_HOST_TAG/bin/aarch64-linux-android-strip', '-s']
...
endian = 'little'
EOF
            sed_in_place 's/dep_libarchive = dependency(.*/dep_libarchive = null_dep/' src/freedreno/meson.build
            sed_in_place 's/dep_libxml2 = dependency(.*/dep_libxml2 = null_dep/' src/freedreno/meson.build
            sed_in_place 's/#elif DETECT_OS_ANDROID && !defined(__cplusplus)/#elif 0/' src/util/perf/cpu_trace.h
