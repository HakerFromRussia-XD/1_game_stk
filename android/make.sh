...
if [ -z "$FLUXARA_DRIFT_MIN_ANDROID_SDK" ]; then
    export FLUXARA_DRIFT_MIN_ANDROID_SDK=21
fi

if [ -z "$FLUXARA_DRIFT_TARGET_ANDROID_SDK" ]; then
    export FLUXARA_DRIFT_TARGET_ANDROID_SDK=35
fi
...
export APP_NAME_RELEASE="Fluxara Drift"
export PACKAGE_NAME_RELEASE="org.fluxaradrift.fluxara_drift"
export PACKAGE_CLASS_NAME_RELEASE="org/fluxaradrift/fluxara_drift"
export APP_DIR_NAME_RELEASE="fluxaradrift"
...
export APP_NAME_BETA="Fluxara Drift Beta"
export PACKAGE_NAME_BETA="org.fluxaradrift.fluxara_drift_beta"
export PACKAGE_CLASS_NAME_BETA="org/fluxaradrift/fluxara_drift_beta"
export APP_DIR_NAME_BETA="fluxaradrift-beta"
...
export APP_NAME_DEBUG="Fluxara Drift Debug"
export PACKAGE_NAME_DEBUG="org.fluxaradrift.fluxara_drift_dbg"
export PACKAGE_CLASS_NAME_DEBUG="org/fluxaradrift/fluxara_drift_dbg"
export APP_DIR_NAME_DEBUG="fluxaradrift-dbg"
...
if [ "$GRADLE_BUILD_TYPE" = "assembleRelease" ]; then
    if [ -z "$FLUXARA_DRIFT_KEYSTORE" ]; then
        echo "Error: FLUXARA_DRIFT_KEYSTORE variable is empty."
        exit
    fi

    if [ ! -f "$FLUXARA_DRIFT_KEYSTORE" ]; then
        echo "Error: Couldn't find $FLUXARA_DRIFT_KEYSTORE file."
        exit
    fi

    if [ -z "$FLUXARA_DRIFT_STOREPASS" ]; then
        echo "Error: FLUXARA_DRIFT_STOREPASS variable is empty"
        exit
    fi

    if [ -z "$FLUXARA_DRIFT_ALIAS" ]; then
        echo "Error: FLUXARA_DRIFT_ALIAS variable is empty."
        exit
    fi
else
    FLUXARA_DRIFT_KEYSTORE="empty"
    FLUXARA_DRIFT_STOREPASS="empty"
    FLUXARA_DRIFT_ALIAS="empty"
fi
...
if [ -d "$DIRNAME/assets/data" ]; then
    if [ ! -f "$DIRNAME/assets/data/fluxaradrift.$PROJECT_VERSION" ]; then
        echo "Error: fluxaradrift.$PROJECT_VERSION doesn't exist in" \
...
echo "    <style name=\"Theme.FLUXARA_DRIFTSplashScreen\" parent=\"android:style/Theme.Holo\">" >> "$STYLES_FILE"
...
# Strings used in fluxara_drift android ui (when extracting game data first time)
...
PO_EXTRACT_ERROR_MSG_STR="Check remaining device space or reinstall Fluxara Drift."
PO_QUIT="po_quit"
PO_QUIT_STR="Quit"
...
sed -i "s/package org.fluxaradrift.*/package $PACKAGE_NAME;/g" \
       "$DIRNAME/src/main/java/FLUXARA_DRIFTEditText.java"

sed -i "s/import org.fluxaradrift.*/import $PACKAGE_NAME.FLUXARA_DRIFTInputConnection;/g" \
       "$DIRNAME/src/main/java/FLUXARA_DRIFTEditText.java"

sed -i "s/package org.fluxaradrift.*/package $PACKAGE_NAME;/g" \
       "$DIRNAME/src/main/java/FLUXARA_DRIFTInputConnection.java"

sed -i "s/import org.fluxaradrift.*.FLUXARA_DRIFTEditText;/import $PACKAGE_NAME.FLUXARA_DRIFTEditText;/g" \
       "$DIRNAME/src/main/java/FLUXARA_DRIFTInputConnection.java"

sed -i "s/package org.fluxaradrift.*/package $PACKAGE_NAME;/g" \
       "$DIRNAME/src/main/java/FluxaraDriftActivity.java"

sed -i "s/import org.fluxaradrift.*/import $PACKAGE_NAME.FLUXARA_DRIFTEditText;/g" \
       "$DIRNAME/src/main/java/FluxaraDriftActivity.java"
...
./gradlew -Pcompile_sdk_version="$COMPILE_SDK_VERSION"   \
          -Pmin_sdk_version="$FLUXARA_DRIFT_MIN_ANDROID_SDK"       \
          -Pcompile_sdk_version="$FLUXARA_DRIFT_TARGET_ANDROID_SDK"\
          -Ptarget_sdk_version="$FLUXARA_DRIFT_TARGET_ANDROID_SDK" \
          -Pstorepass="$FLUXARA_DRIFT_STOREPASS"                   \
          -Pkeystore="$FLUXARA_DRIFT_KEYSTORE"                     \
          -Palias="$FLUXARA_DRIFT_ALIAS"                           \
          -Pndk_version="$FLUXARA_DRIFT_NDK_VERSION"               \
