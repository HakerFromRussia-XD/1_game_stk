...
-- Email  : rx1@posteo.de
-- Version: 3 / 1.000
-- Source : https://online.supertuxkart.net/addons.php?type=tracks
...
- This track was created with Blender v2.79b and the scripts from "blender_26" folder of the SuperTuxKart Media Repository (Revision 18272).
...
- The track was tested with SuperTuxKart v1.2 and v1.3 (and only briefly tested with v1.4 due to local issues with that version).
- modified export script stk_material_parameters.xml (source archive only):
...
- increased reflections since version 2 / internal version 0.991:
	- The reflection settings have be changed because since STK v1.3 reflections have be decreased a lot.
	- If you are still using an older STK 1.x version you will see very strong reflection on track now.
...
- from https://forum.freegamedev.net/viewtopic.php?f=18&t=18044 :
	...
	is under construction, it is open for the intergalactic STK community to race on a colorful temporary speedway. ;)

	I spent a lot of time with this track, but it never was intended to become part of STKs main source, because I am aware 
	...
	it is also suitable for occasional STK players. You most probably will not perfectly master the track on the first laps 
...
- use nitro with on track speed boosts:
	- This instantly further increases your top speed even if STKs speedometer already displays maximum speed.
- force field gates:
...
- long loading times
	 - this is due to STKs driveline calculations
	 - fixed in STK v1.3
- faulty resets
	- If you get off the track and land on or close to another part of the track it can happen that STK resets your cart in 
...
- basketballs
	- STKs basketballs in general are too slow to catch the leader on high speed tracks like this one. 
...
- vertical sections:
	- STKs gravitation / magnet feature appears to have issues on vertical (90° slope) sections, which affects 
...
- precise kart positions:
	- STK appears to have issues determining the exact racing positions of the karts in locations with more than one driveline 
...
- frame rate:
	- The average polygon count in game is around 100000 (some STK standard tracks have more).
	- If the track runs slow on your PC or phone, try lower STK graphics settings.
- game play / design:
	- On very fast as well as on curvy race tracks you can quickly be out of weapon reach for an opponent that gets shot or 
	  crashes. This is a general characteristic of STKs race mode with weapons. If you'd like to eliminate this factor, try 
	...
	- If you don't like super fast racing (in STK), tracks that require some practice or the design of this track, just don't 
...
- Thanks to Heuchi1 for fixing STKs long loading issue and to benau for adding support to increase checkline height 
  as result of the discussions about this track in freegamedev forum and STK IRC channel.
